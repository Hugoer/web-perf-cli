/**
 * CrUX batch audit — save to disk.
 *
 * Fetches CrUX data for multiple URLs and writes each result to its own
 * file under results/crux/. Prints progress and the saved paths when done.
 *
...
async function main() {
    console.log(`Fetching CrUX batch data for ${URLS.length} URLs and saving to disk...\n`);

    const results = await runCruxBatch(URLS, API_KEY, {
        scope: 'page',
        concurrency: 2,
        onProgress,
    });
