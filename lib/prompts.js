...
function getInquirer() {
    if (!_inquirer) {
        _inquirer = require('inquirer').default;
    }
    return _inquirer;
}
    ...
    const { command } = await inquirer.prompt([
        {
            ...
            choices: [
                ...
                { name: 'clean              — Generate AI-friendly .clean.json from existing reports', value: 'clean' },
            ],
        },
    ]);
    return command;
 ...
 */
function parseRunsFlag(runsValue, { reuseBrowser = false } = {}) {
    if (runsValue === undefined || runsValue === null || runsValue === '') {
        return 1;
    }
    const runs = Number(runsValue);
    if (!Number.isInteger(runs) || runs < 1) {
        throw new Error(`--runs must be a positive integer (got "${runsValue}")`);
    }
    if (runs > 1 && reuseBrowser) {
        throw new Error(
            '--runs cannot be combined with --reuse-browser: repeated runs on a shared browser '
            + 'measure the connection pool warming up, not the page. Drop --reuse-browser to sample properly.',
        );
    }
    return runs;
}
...
function parseCruxFormFactors(formFactorStr) {
    ...
    const parts = formFactorStr.split(',').map((s) => s.trim().toLowerCase()).filter(Boolean);
    if (parts.length === 0) {
        return [...DEFAULT_CRUX_FORM_FACTORS];
    }
    for (const name of parts) {
        if (!CRUX_FORM_FACTORS.includes(name)) {
            throw new Error(`Invalid form factor: "${name}". Allowed: ${CRUX_FORM_FACTORS.join(', ')}`);
        }
    }
    return [...new Set(parts)];
}

function parsePsiStrategies(strategyStr) {
    if (!strategyStr) {
        return [...DEFAULT_PSI_STRATEGIES];
    }
    const parts = strategyStr.split(',').map((s) => s.trim().toLowerCase()).filter(Boolean);
    if (parts.length === 0) {
        return [...DEFAULT_PSI_STRATEGIES];
    }
    for (const name of parts) {
        if (!PSI_STRATEGIES.includes(name)) {
            throw new Error(`Invalid strategy: "${name}". Allowed: ${PSI_STRATEGIES.join(', ')}`);
        }
    }
    return [...new Set(parts)];
}
...
async function promptLab(url, options) {
    ...
    // Flag path: profile, network, or device provided via CLI flags — populate runs but do NOT return early
    ...
    // Interactive profile selection: only when no profile/network/device flags were given
    if (resolved.runs.length === 0) {
        ...
        if (selectedProfiles.includes('all')) {
            const allProfileNames = Object.keys(PROFILES);
            const hasCustom = selectedProfiles.includes('custom');
            selectedProfiles = hasCustom ? [...allProfileNames, 'custom'] : allProfileNames;
        }
        ...
        if (selectedProfiles.includes('custom')) {
            ...
            const answers = await inquirer.prompt([
                { type: 'select', name: 'network', message: 'Network throttling:', choices: networkChoices },
                { type: 'select', name: 'device', message: 'Device emulation:', choices: deviceChoices },
            ]);
    ...
    // Blocked URL patterns — prompt if not provided via --blocked-url-patterns flag
    ...
    if (options.stripJsonProps === undefined) {
        assertTTY();
        const { stripJsonProps } = await inquirer.prompt([
            {
                type: 'confirm',
                name: 'stripJsonProps',
                message: 'Allow strip unneeded properties?',
                default: true,
            },
        ]);
        resolved.stripJsonProps = stripJsonProps;
    } else {
        resolved.stripJsonProps = options.stripJsonProps;
    }
        ...
        const { clean } = await inquirer.prompt([
            {
                type: 'confirm',
                name: 'clean',
                message: 'Strip output for AI? (--clean)',
                default: false,
            },
        ]);
        resolved.clean = clean;
    } else {
        resolved.clean = options.clean;
    }
...
async function promptPsi(url, options) {
    ...
    // Resolve strategies: flag > interactive checkbox (TTY) > default (both).
    if (options.strategy) {
        resolved.strategies = parsePsiStrategies(options.strategy);
    } else if (process.stdin.isTTY) {
        const { strategies } = await inquirer.prompt([
            {
                type: 'checkbox',
                name: 'strategies',
                message: 'PSI strategies (both selected by default):',
                choices: [
        ...
        resolved.strategies = strategies.length > 0 ? strategies : [...DEFAULT_PSI_STRATEGIES];
    } else {
        resolved.strategies = [...DEFAULT_PSI_STRATEGIES];
    }

    // Resolve categories: flag > prompt > default (all)
    if (options.category) {
...
async function promptCrux(url, options) {
    ...
    // Resolve form factors: flag > interactive checkbox (TTY) > default (phone+desktop)
    if (options.formFactor) {
        resolved.formFactors = parseCruxFormFactors(options.formFactor);
    } else if (process.stdin.isTTY) {
        const { formFactors } = await inquirer.prompt([
            {
                type: 'checkbox',
                name: 'formFactors',
                message: 'CrUX form factors (phone + desktop selected by default):',
                choices: [
                    { name: 'Phone', value: 'phone', checked: true },
                    { name: 'Desktop', value: 'desktop', checked: true },
                    { name: 'Tablet', value: 'tablet', checked: false },
                ],
            },
        ]);
        resolved.formFactors = formFactors.length > 0 ? formFactors : [...DEFAULT_CRUX_FORM_FACTORS];
    } else {
        resolved.formFactors = [...DEFAULT_CRUX_FORM_FACTORS];
    }
...
async function promptSitemap(url, options) {
    const resolved = { url, depth: options.depth, delay: options.delay, outputAi: options.outputAi };
    ...
    const { input } = await inquirer.prompt([
        {
            type: 'input',
            name: 'input',
            message: 'File, directory, or glob pattern to clean:',
            validate: (v) => {
                if (!v.trim()) {
                    return 'Path is required';
                }
                if (GLOB_CHARS.test(v)) {
                    return true;
                }
                if (fs.existsSync(v.trim())) {
                    return true;
                }
                return `Not found: ${v.trim()}`;
            },
        },
    ]);
...
async function promptLinks(url, options = {}) {
    const resolved = { url, outputAi: options.outputAi };
