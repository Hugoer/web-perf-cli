    ...
    const { command } = await inquirer.prompt([
        {
            ...
            choices: [
                ...
                { name: 'collect            — Extract CrUX data via CrUX API', value: 'collect' },
                { name: 'collect-history    — Historical CrUX data via CrUX API', value: 'collect-history' },
...
async function promptCollect(url, options) {
    assertTTY();
    const resolved = { apiKey: null, urls: [], scope: options.scope || null, concurrency: options.concurrency, delay: options.delay };
...
async function promptCollectHistory(url, options) {
    assertTTY();
    return promptCollect(url, options);
}
