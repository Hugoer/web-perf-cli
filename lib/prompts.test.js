...
describe('promptLab', () => {
    it('should return resolved options when all provided', async () => {
        const result = await prompts.promptLab('https://example.com', { profile: 'low', network: undefined, device: undefined });
        expect(result).toEqual({ url: 'https://example.com', profile: 'low', network: undefined, device: undefined });
        expect(promptSpy).not.toHaveBeenCalled();
    });

    it('should prompt for url when missing', async () => {
        promptSpy.mockResolvedValueOnce({ url: 'https://prompted.com' });
        promptSpy.mockResolvedValueOnce({ profile: 'low' });
        const result = await prompts.promptLab(undefined, { profile: undefined, network: undefined, device: undefined });
        expect(result.url).toBe('https://prompted.com');
    });

    it('should prompt for profile when no profile/network/device given', async () => {
        promptSpy.mockResolvedValueOnce({ profile: 'high' });
        const result = await prompts.promptLab('https://example.com', { profile: undefined, network: undefined, device: undefined });
        expect(result.profile).toBe('high');
    });

    it('should prompt for network and device when profile is empty', async () => {
        promptSpy.mockResolvedValueOnce({ profile: '' });
