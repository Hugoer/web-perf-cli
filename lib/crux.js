...
async function runCruxWithRetry(url, apiKey, { scope = 'page', maxRetries = 2, baseDelayMs = 2000 } = {}) {
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
            return await runCruxAudit(url, apiKey, { scope });
        } catch (err) {
...
async function runCruxAuditBatch(urls, apiKey, { scope = 'page', concurrency = 5, delayMs = 0, onProgress } = {}) {
    const waitForRateLimit = createRateLimiter({ maxRequestsPerSecond: CRUX_MAX_REQUESTS_PER_SECOND });
    const results = [];
    let completed = 0;
    ...
    async function worker() {
        for (const url of iterator) {
            ...
            try {
                ...
                data = await runCruxWithRetry(url, apiKey, { scope });
            } catch (err) {
                error = err.message;
            }
            completed++;
...
async function runCruxBatch(urls, apiKey, { scope = 'page', concurrency = 5, delayMs = 0, onProgress } = {}) {
    ...
    async function worker() {
        for (const url of iterator) {
            ...
            try {
                await waitForRateLimit();
                const data = await runCruxWithRetry(url, apiKey, { scope });
                outputPath = buildFilename(data.url, 'crux');
...
module.exports = { buildRequestBody, runCrux, runCruxBatch, runCruxAudit, runCruxAuditBatch, CRUX_MAX_REQUESTS_PER_SECOND };
