...
async function runCruxAuditBatch(urls, apiKey, { scope = 'page', concurrency = 5, delayMs = 0, formFactors = DEFAULT_CRUX_FORM_FACTORS, onProgress } = {}) {
    ...
    // A 404 means the page or origin is not in the CrUX dataset, which is a fact about the
    // page rather than a failure of the request. Reported separately so it does not fail
    // the run — see docs/specs/SPEC-005.
    return results.map((r) => ({
        url: r.item.url,
        formFactor: r.item.formFactor,
        data: r.data ?? null,
        noData: r.statusCode === 404,
        error: r.statusCode === 404 ? null : r.error,
    }));
}
