...
const {
    extractUrls, decodeXmlEntities, partitionByOrigin, isSitemapIndex, randomizeDelay,
    resolveSitemapUrl,
} = require('./sitemap');
...
describe('partitionByOrigin', () => {
    ...
    it('keeps child sitemaps on the same origin', () => {
        const urls = ['https://example.com/sitemap-1.xml', 'https://example.com/a/sitemap-2.xml'];
        expect(partitionByOrigin(urls, origin)).toEqual({ kept: urls, rejected: [] });
    });

    it('rejects a child sitemap on another host', () => {
        const urls = ['https://example.com/ok.xml', 'https://evil.com/sitemap.xml'];
        expect(partitionByOrigin(urls, origin)).toEqual({
            kept: ['https://example.com/ok.xml'],
            rejected: ['https://evil.com/sitemap.xml'],
        });
    });

    // The vector this closes: an untrusted index steering the crawler at internal hosts.
    it('rejects loopback and link-local targets', () => {
        const urls = ['http://localhost:8080/admin', 'http://169.254.169.254/latest/meta-data/'];
        expect(partitionByOrigin(urls, origin)).toEqual({ kept: [], rejected: urls });
    });

    it('rejects a different scheme on the same host', () => {
        expect(partitionByOrigin(['http://example.com/sitemap.xml'], origin).rejected)
            .toEqual(['http://example.com/sitemap.xml']);
    });

    it('rejects a different port on the same host', () => {
        expect(partitionByOrigin(['https://example.com:8443/sitemap.xml'], origin).rejected)
            .toEqual(['https://example.com:8443/sitemap.xml']);
    });

    // A subdomain is a distinct origin, and cross-host submission is out of spec.
    it('rejects a subdomain of the same registrable domain', () => {
        expect(partitionByOrigin(['https://www.example.com/sitemap.xml'], origin).rejected)
            .toEqual(['https://www.example.com/sitemap.xml']);
    });

    it('rejects a location that does not parse as a URL', () => {
        expect(partitionByOrigin(['not a url', '/relative/sitemap.xml'], origin)).toEqual({
            kept: [],
            rejected: ['not a url', '/relative/sitemap.xml'],
        });
    });

    it('returns empty partitions for an empty list', () => {
        expect(partitionByOrigin([], origin)).toEqual({ kept: [], rejected: [] });
    });
});
