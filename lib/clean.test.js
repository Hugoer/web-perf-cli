...
// Fixture with all Layer 1 keys (must stay in sync with ROOT_KEYS_TO_DROP in clean.js)
...
describe('cleanLabReport', () => {
    ...
    it('drops mainDocumentUrl and finalDisplayedUrl (LH13 redundant fields)', () => {
        const result = cleanLabReport(makeLabReport());
        expect(result).not.toHaveProperty('mainDocumentUrl');
        expect(result).not.toHaveProperty('finalDisplayedUrl');
    });

    it('hoists formFactor from configSettings when absent at root (LH13 compat)', () => {
        // LH13 removed formFactor from root; it lives in configSettings.formFactor now
        const report = makeLabReport({
            formFactor: undefined,
            configSettings: { formFactor: 'mobile', emulatedFormFactor: 'mobile' },
        });
        delete report.formFactor;
        const result = cleanLabReport(report);
