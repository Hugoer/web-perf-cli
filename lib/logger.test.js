...
describe('logger.summary — optional no-data count', () => {
    afterEach(() => {
        vi.restoreAllMocks();
    });

    it('renders two parts when noData is omitted (psi and other callers)', () => {
        const spy = vi.spyOn(console, 'log').mockImplementation(() => {});
        logger.summary(3, 1);
        expect(spy.mock.calls[0][0]).toContain('3 succeeded');
        expect(spy.mock.calls[0][0]).toContain('1 failed');
        expect(spy.mock.calls[0][0]).not.toContain('no data');
    });

    it('renders three parts when noData is supplied', () => {
        const spy = vi.spyOn(console, 'log').mockImplementation(() => {});
        logger.summary(8, 0, 2);
        expect(spy.mock.calls[0][0]).toContain('8 succeeded');
        expect(spy.mock.calls[0][0]).toContain('2 no data');
        expect(spy.mock.calls[0][0]).toContain('0 failed');
    });

    it('still renders the count when noData is zero', () => {
        const spy = vi.spyOn(console, 'log').mockImplementation(() => {});
        logger.summary(5, 0, 0);
        expect(spy.mock.calls[0][0]).toContain('0 no data');
    });
});
