...
describe('runCleanCmd — missing input', () => {
    // Resolving to an empty result and exiting 0 makes a mistyped path indistinguishable
    // from a real no-op for any script or CI step calling the CLI.
    it('rejects when a file path does not exist', async () => {
        const missing = path.join(tmpDir, 'does-not-exist.json');
        await expect(runCleanCmd(missing)).rejects.toThrow(`Input not found: ${missing}`);
    });

    it('rejects when a directory path does not exist', async () => {
        const missing = path.join(tmpDir, 'no-such-dir');
        await expect(runCleanCmd(missing)).rejects.toThrow('Input not found');
    });

    it('resolves with an empty result when a glob matches nothing', async () => {
        const pattern = path.join(tmpDir, 'nothing-*.json');
        const result = await runCleanCmd(pattern);
        expect(result).toEqual({ cleaned: [], skipped: [], errored: [] });
    });

    it('resolves when a directory exists but holds no .json files', async () => {
        const result = await runCleanCmd(tmpDir);
        expect(result).toEqual({ cleaned: [], skipped: [], errored: [] });
    });
});
