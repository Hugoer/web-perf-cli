...
function buildRunSummary(runs, context = {}) {
    ...
    const scored = succeeded.filter((r) => performanceScore(r.report) !== null);
    const scores = scored.map((r) => performanceScore(r.report));
    const medianIndex = selectMedianRun(scores);
    ...
    const summary = {
        ...
        scores,
        median: hasMedian ? scores[medianIndex] : null,
        spread: scores.length > 0 ? { min: Math.min(...scores), max: Math.max(...scores) } : null,
        benchmarkIndex: {
