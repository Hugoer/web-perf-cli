...
function buildRunSummary(runs, context = {}) {
    ...
    // `scores` is index-parallel to `succeeded` (built directly above), and both filters below
    // select on that same null test while preserving relative order — so `scored[i]` and
    // `scoredValues[i]` are the same run, and medianIndex addresses both. Reordering `scores`,
    // or filtering `succeeded` on anything else, silently breaks that pairing and makes
    // medianRun and medianOutputPath point at the wrong run.
