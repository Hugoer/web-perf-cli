...
async function parseSitemaps(sitemapUrl, maxDepth = 3, delayMs = 0, { isFirst = true, origin = null } = {}) {
    ...
    logger.info(`Fetching: ${sitemapUrl}`);
    const xml = await fetchSitemap(sitemapUrl);

    if (!xml) {
        logger.warn(`Could not fetch ${sitemapUrl} — skipping`);
        return [];
    }

    if (isSitemapIndex(xml)) {
        const { kept, rejected } = partitionByOrigin(extractUrls(xml), scope);
        logger.info(`Found sitemap index with ${kept.length} child sitemap(s)`);
        rejected.forEach((childUrl) => {
            logger.warn(`Skipping off-origin child sitemap (expected ${scope}): ${childUrl}`);
        });
    ...
    logger.info(`Found ${urls.length} URL(s) in ${sitemapUrl}`);
    return urls;
}
...
async function runSitemap(url, maxDepth = 3, delayMs = 0) {
    ...
    logger.info(`Extracting URLs from sitemap for: ${origin} (max depth: ${maxDepth})`);
