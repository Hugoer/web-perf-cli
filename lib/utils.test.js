...
describe('utils', () => {
    ...
    describe('normalizeUrlsToOrigins', () => {
        it('should collapse different pages of the same site into one origin', () => {
            const result = normalizeUrlsToOrigins([
                'https://github.com/Hugoer/web-perf-cli',
                'https://github.com/Hugoer/other-repo',
            ]);
            expect(result.origins).toEqual(['https://github.com']);
        ...
        it('should keep distinct origins', () => {
            const result = normalizeUrlsToOrigins(['https://example.com/a', 'https://other.com/b']);
            expect(result.origins).toEqual(['https://example.com', 'https://other.com']);
            expect(result.duplicatesRemoved).toBe(0);
        });

        it('should treat different subdomains and protocols as distinct origins', () => {
            const result = normalizeUrlsToOrigins(['https://example.com', 'https://www.example.com', 'http://example.com']);
            expect(result.origins).toEqual(['https://example.com', 'https://www.example.com', 'http://example.com']);
            expect(result.duplicatesRemoved).toBe(0);
        });

        it('should report which URLs were rewritten to their origin', () => {
            const result = normalizeUrlsToOrigins(['https://example.com/a', 'https://example.com']);
            expect(result.normalized).toEqual([{ from: 'https://example.com/a', to: 'https://example.com' }]);
        });

        it('should assume https for bare hostnames', () => {
            const result = normalizeUrlsToOrigins(['example.com', 'https://example.com']);
            expect(result.origins).toEqual(['https://example.com']);
            expect(result.duplicatesRemoved).toBe(1);
        });

        it('should not report a trailing-slash-only URL as normalized', () => {
            const result = normalizeUrlsToOrigins(['https://example.com/']);
            expect(result.origins).toEqual(['https://example.com']);
            expect(result.normalized).toEqual([]);
        });

        it('should return empty results for an empty list', () => {
            expect(normalizeUrlsToOrigins([])).toEqual({ origins: [], normalized: [], duplicatesRemoved: 0 });
        });
    });
...
// Global fetch rejects with a bare `TypeError: fetch failed` and hangs the real reason off
// `cause`, so an error carrying no statusCode is the normal shape of a network fault.
const fetchFailure = (code) => Object.assign(new TypeError('fetch failed'), {
    cause: Object.assign(new Error('socket hang up'), { code }),
});

describe('withRetry — network failures', () => {
    const noSleep = () => Promise.resolve();

    it('retries a fetch failure whose cause is a transient network code', async () => {
        const fn = vi.fn()
    ...
    it('retries a connect timeout', async () => {
        const fn = vi.fn()
            .mockRejectedValueOnce(fetchFailure('UND_ERR_CONNECT_TIMEOUT'))
            .mockResolvedValue('ok');
        await withRetry(fn, { maxRetries: 2, _sleep: noSleep });
        expect(fn).toHaveBeenCalledTimes(2);
    });

    it('does not retry a hostname that does not resolve', async () => {
        const fn = vi.fn().mockRejectedValue(fetchFailure('ENOTFOUND'));
        await expect(withRetry(fn, { maxRetries: 2, _sleep: noSleep })).rejects.toThrow('fetch failed');
        expect(fn).toHaveBeenCalledTimes(1);
    });

    it('gives up after maxRetries on a persistent network fault', async () => {
        const fn = vi.fn().mockRejectedValue(fetchFailure('ETIMEDOUT'));
        await expect(withRetry(fn, { maxRetries: 2, _sleep: noSleep })).rejects.toThrow('fetch failed');
...
describe('isRetryableError', () => {
    ...
    // An answered request is not transient, however network-ish the error object looks.
    it('lets an explicit status code win over a network code', () => {
        expect(isRetryableError({ statusCode: 404, code: 'ECONNRESET' })).toBe(false);
    });
    ...
    it('reads a code nested deeper in the cause chain', () => {
        const inner = Object.assign(new Error('inner'), { code: 'EAI_AGAIN' });
        const middle = Object.assign(new Error('middle'), { cause: inner });
        expect(isRetryableError(Object.assign(new TypeError('fetch failed'), { cause: middle }))).toBe(true);
    });

    it('does not retry an unrecognised network code', () => {
        expect(isRetryableError(fetchFailure('ECONNREFUSED'))).toBe(false);
        expect(isRetryableError(fetchFailure('CERT_HAS_EXPIRED'))).toBe(false);
    });

    it('does not retry an error carrying neither a status nor a code', () => {
