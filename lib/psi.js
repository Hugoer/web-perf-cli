 ...
 */
async function runPsi(url, apiKey, categories = ['PERFORMANCE', 'ACCESSIBILITY', 'BEST_PRACTICES', 'SEO'], { clean = false } = {}) {
 ...
 */
async function runPsiBatch(urls, apiKey, categories, { concurrency = 5, delayMs = 0, clean = false, onProgress } = {}) {
    ensureCommandDir('psi');
    return runBatch(
        urls,
        (url) => withRetry(() => runPsiAudit(url, apiKey, categories), { label: url }),
