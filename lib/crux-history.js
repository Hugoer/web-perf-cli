...
async function runCruxHistory(rawUrl, apiKey, { scope = 'page' } = {}) {
    ...
    const outputPath = buildFilename(url, 'crux-history');
...
async function runCruxHistoryWithRetry(url, apiKey, { scope = 'page', maxRetries = 2, baseDelayMs = 2000 } = {}) {
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
            return await runCruxHistory(url, apiKey, { scope });
        } catch (err) {
    ...
    async function worker() {
        for (const url of iterator) {
            ...
            try {
                ...
                outputPath = await runCruxHistoryWithRetry(url, apiKey, { scope });
            } catch (err) {
                error = err.message;
            }
            completed++;
...
module.exports = { runCruxHistory, runCruxHistoryBatch, CRUX_HISTORY_MAX_REQUESTS_PER_SECOND };
